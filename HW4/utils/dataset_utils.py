...
class RainSnowTrainDataset(Dataset):
    """
    Return: ([img_name, weather_id], degraded_patch, clean_patch)
            weather_id: 0 = rain, 1 = snow
    """
    def __init__(self, args, phase="train"):
        ...
        self.degraded_dir = os.path.join(root, phase, "degraded")
        self.clean_dir    = os.path.join(root, phase, "clean")

        self.samples = self._scan_files()
        self.crop    = Compose([
            ToPILImage(), RandomCrop(args.patch_size)
        ])
        self.toTensor = ToTensor()

    def _scan_files(self):
        samples = []
        for name in os.listdir(self.degraded_dir):
            if not name.lower().endswith((".png", ".jpg", ".jpeg")):
                continue
            weather = 0 if name.startswith("rain") else 1  # 0: rain, 1: snow
            degraded_path = os.path.join(self.degraded_dir, name)

            # rain-1.png  ➜  rain_clean-1.png
            base = name.split('.')[0]            # rain-1  / snow-832
            clean_name = base.replace("rain", "rain_clean").replace(
                "snow", "snow_clean") + ".png"
            clean_path = os.path.join(self.clean_dir, clean_name)
            assert os.path.exists(clean_path), f"Missing GT: {clean_path}"
            samples.append((degraded_path, clean_path, weather))
        return samples
    ...
    def __getitem__(self, idx):
        de_path, cl_path, w_id = self.samples[idx]
        de_img = np.array(Image.open(de_path).convert("RGB"))
        cl_img = np.array(Image.open(cl_path).convert("RGB"))
        ...
        if self.phase == "val":
            ...
            y = (H - self.args.patch_size) // 2
            x = (W - self.args.patch_size) // 2
            de_patch = de_img[y:y+self.args.patch_size, x:x+self.args.patch_size]
            cl_patch = cl_img[y:y+self.args.patch_size, x:x+self.args.patch_size]
        else:
            # random crop + augmentation
            y = random.randint(0, H - self.args.patch_size)
            x = random.randint(0, W - self.args.patch_size)
            de_patch = de_img[y:y+self.args.patch_size, x:x+self.args.patch_size]
            cl_patch = cl_img[y:y+self.args.patch_size, x:x+self.args.patch_size]
            de_patch, cl_patch = random_augmentation(de_patch, cl_patch)

        return [os.path.basename(de_path), w_id], self.toTensor(de_patch), self.toTensor(cl_patch)
